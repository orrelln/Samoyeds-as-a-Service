...
class QueueItem:
    def __init__(self, image_path, idx, status):
    ...
    async def classify(self):
        with tf.Session() as sess:
            ...
            while True:
                ...
                if not tf.gfile.Exists(qitem.image_path):
                    pass
                try:
                    image_data = tf.gfile.FastGFile(qitem.image_path, 'rb').read()
                except Exception:
                    qitem.status = 500
                ...
                sorted_predictions = predictions.argsort()[-5:][::-1]

                for node_id in sorted_predictions:
                    readable_string = self.node_lookup.id_to_string(node_id)
                    readable_string = readable_string.split(',')[0].split(' ')[0]
                    score = predictions[node_id]
                    qitem.predictions.append((readable_string, score))
